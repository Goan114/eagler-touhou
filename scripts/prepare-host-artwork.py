...
def _default_artwork(game: str, name: str, roots: dict[str, Path]) -> bytes:
    root = roots.get(game)
    if root is None:
        raise FileNotFoundError(
    ...
    for game in games:
        for name in ARTWORK_BY_GAME[game]:
            ...
            if override is not None and override.is_file():
                ...
                try:
                    raw = _default_artwork(game, name, roots)
                    if name.endswith(".webp"):
...
def _main() -> None:
    ...
    parser.add_argument("--games", required=True, help="comma-separated subset of th06,th07,th08")
    ...
    if not games or len(games) != len(set(games)) or any(game not in ARTWORK_BY_GAME for game in games):
        parser.error("--games must be a non-empty unique subset of th06,th07,th08")
    roots = {
