...
export function runtimeFileNames(game, files) {
  ...
  for (const name of names) {
    if (!isCanonicalRelativePath(name) || !(
      ...
      /^runtime\/[a-z0-9._-]+\.(?:js|mjs)$/i.test(name) ||
      /^vendor\/(?:v86\.(?:js|wasm|LICENSE)|th10-game\.wasm|ogg-vorbis-decoder\.(?:mjs|LICENSE|provenance\.json)|ogg-vorbis\.LICENSE|ogg\.LICENSE)$/.test(name) ||
      /^fonts\/(?:blend\.bin\.gz|128\/\d+-\d+\.json\.gz)$/.test(name)
