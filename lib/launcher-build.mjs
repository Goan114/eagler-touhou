...
export function isMappedBrowserPublicationPath(value) {
  const path = normalizePublicPath(value);
  return isLauncherBuildOutputPath(path) || browserFacadePaths.has(path) || ["index.html", "en.html", "styles.css", "touch-guide.css"].includes(path);
}
...
export function resolveBrowserPublicationSource(value) {
  ...
  if (hasTypeScriptSource && (path === "assets/launcher/app.mjs" || /^assets\/launcher\/[^/]+-[A-Z0-9]{8}\.mjs$/.test(path) ||
      ["index.html", "en.html", "styles.css", "touch-guide.css"].includes(path))) {
...
export async function ensureLauncherBuild({ force = false } = {}) {
  ...
  // FAQ Markdown is authoritative. Every source build refreshes its deployable
  // HTML so maintainers never have to remember a second content-build step.
