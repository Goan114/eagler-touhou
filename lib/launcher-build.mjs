...
export function resolveBrowserPublicationSource(value) {
  ...
  if (isLauncherBuildOutputPath(path)) {
    const cached = resolve(buildRoot, path);
    if (existsSync(sourceRoot) || existsSync(cached)) return cached;
  }
  if (browserFacadePaths.has(path)) return resolve(project, "src", "browser-facades", path);
...
async function materializeBrowserModuleOverlay() {
  ...
  for (const path of modules) {
    ...
    await cp(resolveBrowserPublicationSource(path), target);
  }
}
...
export async function ensureLauncherBuild({ force = false } = {}) {
  // Packaged Host Kits intentionally contain compiled browser modules but not
  ...
  if (!await exists(sourceRoot)) {
    if (!(await Promise.all(generatedOutputRoots.map(exists))).every(Boolean)) {
