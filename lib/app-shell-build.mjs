...
export async function buildAppShell({
  ...
  const manifestConfig = {
    ...
    // Workbox's default MD5 revision is a cache key, not Web Crypto integrity.
    // Both passes must publish the same SHA-256 so the worker can reject a
    // mixed deployment/HTML fallback before labelling bytes as a new version.
