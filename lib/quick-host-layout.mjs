...
export const QUICK_HOST_SCHEMA = "eagler-touhou/quick-host/1";
export const QUICK_HOST_GAMES = Object.freeze(Object.keys(PRODUCT_GAMES));
...
function normalizeMusic(music) {
  ...
  if (modes.some(mode => !["midi", "ogg"].includes(mode))) {
    throw new Error("quick host supports only midi or midi,ogg; use the advanced deployment entry for WAV/custom music layouts");
  }
...
export function quickHostPaths(root) {
  ...
  return Object.freeze({
    ...
    games: Object.freeze(Object.fromEntries(QUICK_HOST_GAMES.map(game => [game, resolve(gamesRoot, game)]))),
  });
}

export async function inspectQuickHost(root, { music = ["midi", "ogg"] } = {}) {
  ...
  for (const game of QUICK_HOST_GAMES) {
