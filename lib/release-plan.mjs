...
export function normalizeFormalReleaseInput(input, { inputDirectory, workspace, windir = "C:/Windows" } = {}) {
  ...
  if (input.externalResourceIndex != null) {
    if (typeof input.externalResourceIndex !== "string" || !input.externalResourceIndex) {
      throw new Error("externalResourceIndex must be a non-empty path");
    }
    externalResourceIndex = resolve(inputDirectory, input.externalResourceIndex);
  }
  return Object.freeze({ games, prepare: Object.freeze(prepare), externalResourceIndex });
}
