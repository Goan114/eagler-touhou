...
for (const game of ["th06", "th07"]) {
  ...
  requireMatch(player, /\(!replayPlayback\s*&&[\s\S]*?Touch::GetPlayerDelta\(&touchDx, &touchDy\)/,
    `${game} live touch can never leak into replay playback`);
  requireMatch(player, /else if \(!replayPlayback\s*&&[\s\S]*?Touch::GetFreeJoystickVector\(&joystickX, &joystickY\)/,
    `${game} live free joystick can never leak into replay playback`);
