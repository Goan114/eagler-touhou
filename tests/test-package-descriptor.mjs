...
assert.equal(validatePackageDescriptor(structuredClone(descriptor)).game, "th08",
  "package schema must accept registered Product Catalog game ids");
const unknownProduct = structuredClone(descriptor);
unknownProduct.game = "th99";
assert.throws(() => validatePackageDescriptor(unknownProduct), /header/,
  "package schema must not create a second, broader product registry");
