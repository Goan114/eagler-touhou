...
const entries = [
  ...
  ["game.select.th06", "选择 东方红魔乡", "Select Touhou 6"],
  ["game.select.th07", "选择 东方妖妖梦", "Select Touhou 7"],
  ["game.select.th08", "选择 东方永夜抄", "Select Touhou 8"],
  ["game.select.th06mp", "选择 东方红魔乡 联机版", "Select Touhou 6 multiplayer"],
  ["game.select.th07mp", "选择 东方妖妖梦 联机版", "Select Touhou 7 multiplayer"],
  ...
  ["settings.actionsNeedLaunch", "先在浏览器中打开一次游戏，才能使用此功能", "Open the game once in this browser before using this feature"],
...
export const UI_MESSAGES = Object.freeze(Object.fromEntries(UI_LOCALES.map((locale, index) => [
  locale,
  Object.freeze(Object.fromEntries(entries.map(([key, zh, english]) => [key, index === 0 ? zh : english]))),
])));
...
export function resolveUiLocale(value) { return /^zh(?:-|$)/i.test(String(value || "")) ? "zh-CN" : "en"; }
export function detectUiLocale() {
  try { const saved = safeStorage()?.getItem(UI_LOCALE_STORAGE_KEY); if (UI_LOCALES.includes(saved)) return saved; } catch {}
  for (const value of globalThis.navigator?.languages || [globalThis.navigator?.language]) if (value) return resolveUiLocale(value);
  return "zh-CN";
}
export function getUiLocale() { return currentLocale; }
export function t(key, params = {}) {
  const value = UI_MESSAGES[currentLocale][key] ?? UI_MESSAGES["zh-CN"][key];
  if (value == null) { globalThis.console?.warn?.(`Missing UI translation: ${key}`); return key; }
  return String(value).replace(/\{([A-Za-z0-9_]+)\}/g, (_, name) => String(params[name] ?? `{${name}}`));
}
export function applyStaticTranslations(root = globalThis.document) {
  ...
  const bindings = [["aria-label", "i18nAriaLabel"], ["title", "i18nTitle"], ["placeholder", "i18nPlaceholder"], ["alt", "i18nAlt"]];
  for (const [attribute, property] of bindings) {
    const selector = `[data-${property.replace(/[A-Z]/g, value => `-${value.toLowerCase()}`)}]`;
    for (const element of root.querySelectorAll(selector)) element.setAttribute(attribute, t(element.dataset[property]));
  }
}
export function setUiLocale(value, { persist = true, notify = true } = {}) {
  ...
  if (typeof document !== "undefined") {
    ...
    const select = document.querySelector("#uiLanguageSelect"); if (select) select.value = currentLocale;
...
export function initUiLocale() {
  ...
  if (select && !bound) { bound = true; select.addEventListener("change", event => setUiLocale(event.currentTarget.value)); }
  return currentLocale;
}
export function validateUiCatalogs() {
