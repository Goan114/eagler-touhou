...
A formal release first generates and verifies the hosted site, then derives the import site and offline ZIPs from **that same hosted build**. It never re-reads unrelated historical artifact identities.
