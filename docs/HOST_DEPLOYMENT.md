...
`dist/site/app-shell-sw.js` is deployment-specific build output.
...
`app-shell-sw.js`; only `src/app-shell-sw.js` is source. Development serving,
...
`deploy/nginx-eagler-touhou.conf` is the public example for operators who
manually serve an already assembled static site. Adjust its `root` and
`server_name`, then configure TLS using your normal nginx/certificate workflow.
The Linux first-install bundle instead renders its owned
`deploy/linux-first-install/nginx-site.conf.template`, which also carries the
installer's migration and health endpoints; operators should not mix the two
configuration paths.
...
The maintained server sources live only in this repository:
...
TH06/TH07 Runtime repositories consume this service during integration tests;
they do not own separate relay copies.
...
For a self-managed coturn instance, copy `server/coturn.env.example` to a
server-only environment file, replace the example realm/secret/host values,
then render a coturn configuration with:

```text
node server/render-coturn-config.cjs --output /etc/turnserver.conf
```

The renderer deliberately rejects a shared secret shorter than 24 bytes. Its
baseline uses coturn REST-secret authentication, disables the coturn CLI and
multicast peers, blocks common private/reserved peer destinations, and applies
bounded allocation and bandwidth quotas. TURN/TLS is optional and is enabled
only when both certificate and private-key paths are configured.
