...
function localAssetKey(key, origin = "https://local.invalid") {
  try {
  ...
  if (db) {
    try {
      if (db.objectStoreNames.contains(LOCAL_ASSET_STORE)) {
        ...
        const value = await requestResult(transaction.objectStore(LOCAL_ASSET_STORE).get(localAssetKey(key, origin)));
  ...
  if (db) {
    try {
      if (db.objectStoreNames.contains(LOCAL_ASSET_STORE)) {
        ...
        transaction.objectStore(LOCAL_ASSET_STORE).delete(localAssetKey(key, origin));
